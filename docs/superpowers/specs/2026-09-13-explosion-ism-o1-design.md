...
`scripts/profile.ps1 -Bullets 5000 -Frames 720`, 3런. 수치는 `scripts/profile-stats.ps1`.

| 런 | 동시 폭발 | Draws mean | Draws p99 | Frame mean | RT mean | Translu mean | Translu p99 |
|---|---:|---:|---:|---:|---:|---:|---:|
| `e149_off` (폭발 꺼짐, 기준선) | 0 | 225 | 259 | 5.78 | 5.76 | 0.19 | 0.61 |
| `e149_nobudget` (`ExplosionBudget 0`) | 85~95 | 229 | 263 | 5.73 | 5.71 | 0.18 | 0.54 |
| `e149_budget256` (출하 설정) | 78~83 | 232 | 282 | 5.54 | 5.53 | 0.16 | 0.34 |

세 런 모두 `버림=0` 이다 — 예산 256 에서도 걸리지 않았다(동시 최대 83).

**판정: 상수화 성공.** 동시 폭발이 0 → 90 으로 올라도 드로우콜은 225 → 229(+4). 변경 전 같은 조건(예산 OFF)이 **680 / p99 780** 이었다(#147). 동시 개수와 드로우콜이 무상관이다.

**층당 드로우콜 실측 ≈ 2.** 두 층 합계 +4 (225→229). 예상(층당 2)과 일치한다. `budget256` 이 +7 인 것은 런 간 노이즈 범위다(동시 개수가 오히려 더 적다).

**오버드로우: 이 조건에서는 관측되지 않았다.** §8 은 동시 개수가 늘면 `Translu` 가 선형으로 오를 것으로 봤지만, 동시 90 에서 `Translu` 는 오히려 기준선보다 낮다(0.19 → 0.18, p99 0.61 → 0.54). RT·Frame 도 세 런이 노이즈 안에서 같다(Frame 5.54~5.78). 동시 90 은 오버드로우가 프레임에 보이기에는 아직 적다는 뜻이고, `re.Fx.ExplosionBudget 256` 의 근거가 되는 무너지는 지점은 **이 하네스에서 찾지 못했다** — 예산은 상한이 아니라 미검증 안전망으로 남는다.

**스크린샷 판정** (`re.Debug.ScreenshotFrame 900`, 실 RHI 1600x900, 동시 63~68): 플레이어·보스 주변에 폭발이 보이고, 링이 바닥에 묻히지 않았으며, 링이 사라지는 Z 스케일 함정도 재현되지 않았다. 반투명 정렬 아티팩트는 눈에 띄지 않아 `BLEND_MASKED` 폴백을 쓰지 않았다. 다만 코어가 블룸과 `Strength 3.0` 때문에 주황보다 흰색에 가깝게 읽힌다 — 머티리얼 파라미터라 코드 변경 없이 조정 가능하다.
