...
| `[Move] rejected: off-navmesh` | **실제 목표 좌표에 대해 0건** |

마지막 항목은 주의가 필요하다. 이동 프로브는 오프메시 거부 경로를 확인하려고 **맵 밖 좌표(100000, 100000)를 일부러 한 번 요청한다.** 그 좌표를 지목한 거부 로그는 정상이며 오히려 거부 경로가 살아있다는 증거다 — 판정에서 제외해야 한다. 이 구분을 안 하면 항상 실패한다.
