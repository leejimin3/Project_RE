...
> **[2026-08-16 승격 — #112]** 아래 결정 3의 게이트("1·2번 측정 후 부족하면 그때 승격")가 충족돼 구현했다.
>
> 실측: 데디 + 실 RHI 클라에서 서버 폰이 **0.5초에 14.9uu** 만 갔다(스탠드얼론 212.5). 서버 폰 속도는 전 구간 0 — 서버가 스스로 움직이지 못하고 클라 `ServerMove` 가 올 때만 찔끔 전진했다. 체감 속도 약 62uu/s 로 설계값 600uu/s 의 **1/10** 이다.
>
> 구현 후: **0.5초에 234.3uu** — 스탠드얼론과 같은 수준.
>
> 이 문서의 1~3절 분석(서버가 `PerformMovement` 를 안 부른다, 패스팔로잉이 거기 얹혀 있다)은 그대로 유효하다. 바뀐 것은 결론뿐이다.
