...
| 부하 | 근거 |
|---|---|
| **1600발** | 측정 당시의 실제 게임플레이 정상상태. `BulletsPerShot=16` × (`BulletLifetime=15` / `BossFireInterval=0.15`) = 16 × 100. **#97 에서 `BulletsPerShot` 이 48 로 올라 현재 게임플레이 부하는 약 4,800발이다** — 아래 1600발 표는 그 시점의 측정으로 읽어라 |
