...
> **[2026-08-16 폐기 — #102]** 위 문단의 계약은 더 이상 유효하지 않다. 지금은 대쉬 중인 플레이어도 목록에 담고 `FREHitTarget::bInvulnerable` 로 표시한다. 소비자가 데미지만 건너뛰고 **탄환은 소멸시킨다.** 데미지 면제 책임이 수집부에서 소비부로 옮겨갔다. 아래 "전원이 대쉬 중이면 `Targets` 가 비고 조기 반환한다"도 같은 이유로 성립하지 않는다.
