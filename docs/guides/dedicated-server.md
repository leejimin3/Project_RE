...
`dedi-verify.ps1` 은 **스테이징 서버 exe 가 `Source/` 최신 수정본보다 낡으면 실패한다.** 매번 재쿡하지는 않는다 — 거짓 통과만 막는다.

| 무엇을 고쳤나 | 재스테이징 |
|---|---|
| 서버에서 도는 코드 (`Mass/` 시뮬·판정, `TakeDamage`, GameMode, 복제 RPC) | **필요** |
| `Config/*.ini` | **필요** — pak 안에 들어간다 |
| 클라 전용 (렌더 프로세서, FX, UI, 머티리얼) | 불필요 — `-SkipStaleCheck` |
| 문서·스크립트 | 불필요 — `-SkipStaleCheck` |

**mtime 기반이라 오탐이 난다.** `git checkout` · `git stash pop` 은 내용이 그대로여도 파일 수정 시각을 갱신한다 — 브랜치를 옮기면 서버 코드를 안 고쳤어도 낡음 판정이 뜬다. 안전한 방향의 오탐이다(통과를 막지, 거짓 통과를 만들지 않는다). 서버가 무관한 것이 확실하면 `-SkipStaleCheck` 로 넘긴다.

`-SkipStaleCheck` 는 **서버가 무관하다고 판단했을 때만** 쓴다. 습관적으로 붙이면 #103 이 그대로 돌아온다 — 서버 exe 만 조용히 낡기 때문에 전 항목 PASS 를 받고도 서버 변경이 한 번도 실행되지 않는다(#98 에서 하루 반 낡은 exe 로 16항목 PASS 를 받았다).
