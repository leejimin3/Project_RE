...
> **⚠️ 무효 갱신(#50, 2026-08-14): 아래 표의 GPU·프레임타임·RenderThread·드로우콜 숫자는 쓰지 마라.**
> 이 측정은 캡처가 엔진 부팅 시점에 시작하던 시절의 것이라, 17.9초 창 중 13.3초가 레벨 로드·셰이더 컴파일
> 구간이었다(#88이 밝히고 고쳤다). 재측정 결과 Mass 5000은 GPU **38.61 → 5.28 ms**, 프레임 **39.54 → 6.18 ms** 다.
> `BulletLifetime` 을 당시 값(3.0)으로 되돌려도 재현되지 않으므로 밸런스 변경 탓이 아니다.
> **게임 스레드·드로우콜 축의 결론(Mass가 왜 나은가)은 유효하며 재측정에서 오히려 강해졌다** —
> 현재 Actor 5000은 GT 16.99 ms로 60fps 게이트를 자력으로 넘지 못한다.
> 현행 정본: `docs/profiling/M6-gpu-breakdown.md`
